...
- **实时事件流** — 通过 Wails Events 实现 `agent:timeline` 统一事件流，前端实时展示 Agent 活动
- **会话持久化** — 完整的会话管理，包含消息历史和富文本 timeline events 存储
...
│   │   ├── chat.go                  #   ChatService：Agent 生命周期、消息收发、流式输出
│   │   ├── sandbox_svc.go           #   SandboxService：连接/断开/重连、健康监控
│   │   ├── session_svc.go           #   SessionService：会话 CRUD
...
│       ├── style.css                # 全局样式
│       ├── components/
│       │   ├── chat/                #   聊天面板、消息气泡、中断对话框、计划面板、Todo 看板
│       │   ├── layout/              #   三栏布局、头部、侧边栏
...
└── sessions/
    └── {session-id}/
        ├── session.json       # 会话元数据
        ├── messages.json      # 对话消息历史（Eino schema）
        └── display.json       # 富文本展示数据（timeline events）
```

## 文档

- **技术文档**：`doc/` — 项目级文档 + 文件级技术说明
- **规划文档**：`plan/` — 项目未来更新方向
