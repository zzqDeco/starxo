...
Starxo uses Wails platform-native shell settings at build time. macOS uses a unified hidden titlebar and system appearance, Windows follows the system theme with Mica where available, and Linux uses a conservative GTK/WebKit fallback. The Vue UI follows system light/dark mode and applies platform-specific design tokens. macOS builds use bundle id `com.starxo.app` and declare Local Network access because Starxo connects to LAN sandbox hosts over SSH; allow the system prompt if you use `192.168.x.x` or `.local` remotes. If a signed macOS app reports `no route to host`, `network is unreachable`, `operation not permitted`, or `permission denied` for a LAN address while Terminal SSH still works, use the in-app SSH diagnostics card, enable Starxo in System Settings > Privacy & Security > Local Network, then quit and reopen the app. macOS does not provide a reliable `tccutil` reset for Local Network state; for stale development test state, retest from a new macOS user account or a VM snapshot. If you previously tested an older `com.wails.starxo` build, allow the prompt again for the new bundle identity.
