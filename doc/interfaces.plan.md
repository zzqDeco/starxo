...
> 所属项目: Eino Coding Agent (starxo) | 文档类型: 接口说明

---

## 一、外部能力概述

Eino Coding Agent 的所有外部接口均通过 **Wails v2 绑定机制** 暴露。前端 (Vue 3) 通过 Wails 自动生成的 TypeScript 绑定调用 Go 后端方法，后端通过 Wails 事件系统向前端推送实时数据。
