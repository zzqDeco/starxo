...
- 平台原生窗口壳配置位于 `main.go`，但 release 产物仍依赖 `wails.json` 的应用名、输出名、版本元数据和 `build/` 平台资源。
...
- 修改 platform icon、manifest、Info.plist 资源路径时，需同步 `.github/workflows/release.yml` 的平台产物检查步骤。
