...
## 2. 核心职责
- 验证 `SessionService.SaveSessionByID(...)` 在 save-time pruning 后仍能保留 deferred MCP discovery history，并验证 session switch 只恢复目标 session 的 todo bucket。
...
- 内存中的 session-scoped todo 快照

## 4. 关键测试覆盖
- 在 `plan` mode 下保存 session 时，先前发现的 read-only / read-write deferred tools 都会被保留
- 保存后的 discovery 顺序稳定
- 成功落盘后内存态 discovery 不会被误删
- 切换 session 时从目标 `SessionData.RuntimeContextCompact.Todos` 恢复对应 bucket，且不写入全局兼容 bucket
...
## 6. 变更影响面
- 保护 `SaveSessionByID` 的 fail-open discovery 持久化主线，以及 session-scoped todo restore 主线。
