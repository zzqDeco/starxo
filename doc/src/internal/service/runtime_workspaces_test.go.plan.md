...
- `DiffWorktree` 返回 status/stat/patch 和单独的 untracked patch，并按 `max_bytes` 截断 patch。
- `MergeWorktree` 会提交 worktree 修改、merge 到 parent、可选 remove worktree，并恢复默认 workspace。
