...
- `MergeWorktree` 将 prepare 和 merge 分成两次远端调用；如果 merge 调用返回非零 exit code 且像 Git conflict，会先读取 `diff --name-only --diff-filter=U`，再对原 workspace 执行 `git merge --abort || true`，返回 `action=merge_conflict` 的结构化恢复结果并保留 active worktree。
- 非冲突 merge 失败仍会执行 `git merge --abort || true` 后返回错误，避免 parent workspace 留在中间状态。
