...
- 输出结果:
  ...
  - 通过 `AgentContext.OnToolEvent` 回调发射事件（参数: agentName, eventType, toolName, toolArgs, toolID, result）
...
- 内部方法:
  ...
  - `(*eventEmittingTool) InvokableRun(ctx, argumentsInJSON, opts...) (string, error)`: 生成唯一 callID（基于纳秒时间戳），发射 tool_call 事件，执行内部工具，发射 tool_result 事件（错误时结果为 "Error: ..."）
- Wails 绑定方法: 无
