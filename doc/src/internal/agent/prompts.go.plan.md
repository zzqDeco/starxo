...
- 结构体/接口定义: 无
- 导出函数/方法:
  - `DeepAgentPrompt(ac AgentContext) string`: 生成核心代理提示词，定义直接工具、Runtime V2 core tools、ToolSearch、动态 `Agent` tool 和子 agent 类型选择规则
  - `DeepAgentPlanPrompt(ac AgentContext) string`: 生成计划模式提示词，强调先规划、只使用 read/search/todo/tool_search，必须通过 `ExitPlanMode` 获得批准后才执行写入或 shell
...
- 关键约束补充:
  ...
  - Runtime V2 直接工具说明：
    ...
    - `Agent` 是默认子任务委派入口，可选择 `general`、`code_writer`、`code_executor`、`file_manager`、`reviewer`；耗时任务可使用 `background=true`
