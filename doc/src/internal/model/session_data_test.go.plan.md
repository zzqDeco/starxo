...
## 2. 核心职责
- 验证 `SessionData` 对旧版持久化数据的向后兼容性，以及 normalize copy / permission grants / permission audit / runtime compact 语义。
...
- 缺失字段会回退为空集合，而不是破坏读取
- normalize 会复制 plan state、`PermissionGrants` 和 `PermissionAudit`，调用方修改 normalized copy 不会污染原始输入
- normalize 会深拷贝 `RuntimeContextCompact` 的 discovered tools、delta state、grants、permission audit、tasks、task graph items、file reads、diff summaries、todos、plan 和 workspace
