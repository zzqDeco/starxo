...
## 2. 核心职责
- 该文件实现了应用配置的持久化存储层 `Store`，负责将 `AppConfig` 以 JSON 格式读写到用户主目录下的 `~/.eino-agent/config.json` 文件。提供线程安全的配置读取、更新和保存操作，支持原子性的 `Update` 方法（读-改-写一体化）。首次创建时若配置文件不存在则使用默认配置。
- 该文件的变更应与项目级规则文档和接口文档保持一致。

## 3. 输入与输出
- 输入来源: 文件系统 `~/.eino-agent/config.json`；`Update` 方法接收修改函数 `func(*AppConfig)`
...
- 配置文件目录名 `.eino-agent` 为硬编码，后续可考虑通过环境变量配置。
