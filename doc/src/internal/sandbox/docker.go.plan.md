...
- 导出函数/方法:
  ...
  - `CreateContainer(ctx) error` — 创建并启动新容器
  ...
  - `ClearContainer()` — 清零 containerID 和 containerName（用于 DetachContainer 场景，容器保持运行但本地引用清除）
...
## 6. 变更影响面
- `internal/sandbox/manager.go` — SandboxManager 创建并编排 RemoteDockerManager，`DetachContainer` 调用 `ClearContainer`
...
- `ClearContainer` 仅清除本地引用，不操作远程容器。用于容器切换场景。
