...
- **Wails 绑定调用**: ListSessionsEnriched, CreateSession, SwitchSession, DeleteSession, RenameSession, GetActiveSession, GetActiveSessionMessages, SaveChatDisplay, LoadChatDisplay

## 5. 依赖关系
- 内部依赖: `@/types/session` (Session)
- 外部依赖: `pinia` (defineStore)、`vue` (ref, computed)
- Wails 绑定: `wailsjs/go/service/SessionService` (9 个方法)
...
- `saveChatDisplay`/`loadChatDisplay` 使用 JSON 字符串进行序列化，需注意大消息列表的性能。
