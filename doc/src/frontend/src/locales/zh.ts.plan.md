...
## 4. 关键实现细节
- **翻译键分组**:
  - `common` — 通用操作文本 (连接、断开、保存、取消等)
  - `header` — 顶部导航栏 (标题、面板切换)
  - `sidebar` — 侧边栏 (新对话、会话管理、容器状态)
  - `chat` — 聊天区域 (标题、空状态提示、提示卡片、模式切换 modeLabel/modeDefault/modePlan)
  - `input` — 输入区域 (占位符、附件、发送提示)
  - `message` — 消息展示 (复制、参数、结果、展开、thinking="正在思考..."、subagent 状态文案、tool 横条摘要文案)
  - `interrupt` — 中断对话框 (标题、输入提示、选项、每题输入占位符 answerPlaceholder)
...
## 6. 变更影响面
- 新增/修改翻译键需同步英文语言包 `en.ts`
- 翻译键被所有使用 `t()` 的组件引用
- 键路径变更需全局搜索替换所有 `t('old.key')` 调用

## 7. 维护建议
- 修改该文件后，同步更新项目级 `implementation.plan.md` 与相关规则文档。
- 新增 UI 文本时需同时在 zh.ts 和 en.ts 中添加对应翻译。
- 保持翻译键的层级结构与组件模块结构一致，便于查找。
- 默认语言为 zh（见 locales/index.ts），新增语言需在 index.ts 中注册。
