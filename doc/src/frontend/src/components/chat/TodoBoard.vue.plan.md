...
## 4. 关键实现细节
- **Props 定义**: `todos: TodoItem[]`、`compact?: boolean`（默认 false，compact 模式下默认折叠 DAG 层，仅显示头部进度信息，点击可展开）
...
- **模板结构**: 头部 (标题 + 百分比 + 进度条 + 状态统计 + compact 模式下的展开/折叠图标 ChevronDown/ChevronForward) → DAG 分层列表 (层间有箭头连接器，compact 模式下默认隐藏，isExpanded 时显示)，每项显示状态图标 + 标题 + 依赖标签 + ID
- **样式**: in_progress 状态有呼吸发光动画 (todoGlow)，done 状态标题有删除线

## 5. 依赖关系
- 内部依赖: 无
- 外部依赖: `vue` (computed, ref)、`naive-ui` (NIcon)、`@vicons/ionicons5` (CheckmarkCircle, EllipseOutline, Reload, CloseCircle, LockClosed, ArrowForward, ChevronDown, ChevronForward)
