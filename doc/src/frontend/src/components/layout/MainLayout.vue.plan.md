...
- 负责 `runtime | workspace | terminal | tasks` 四态 inspector 互斥、响应式 sheet、命令面板、面板宽度拖拽持久化和设置面板显隐。
...
- 内部状态:
  ...
  - `runtimeDockWidth` — runtime/terminal/tasks inspector 宽度（默认 344）
  - `workspaceInspectorWidth` — workspace inspector 宽度（默认 620）
  - `inspectorMode` — 右侧 inspector 模式，`runtime` / `workspace` / `terminal` / `tasks`
- 响应式计算:
  ...
  - `isBelow768`: 小于等于 768 时进入 single-column/sheet mode；该断点依赖 `main.go` 的 `MinWidth: 768`
- 布局结构:
  - `Sidebar` + 左分割条
  - 中央 `Header + ChatPanel`
  - 桌面端右侧 inspector 在 `ContainerDock`、`WorkspacePanel`、`TerminalPanel`、embedded `RuntimeTasksPanel` 间互斥切换
  - 窄屏下统一通过 responsive inspector sheet 展示当前 inspector mode
  - SettingsPanel / CommandPalette / ContainerDock / RuntimeTasksPanel 使用 async component 降低首包
  - `onWorkspaceOpenPath()` 打开 workspace inspector；路径选择由 WorkspacePanel 消费 pending path
- macOS 样式:
...
## 6. 变更影响面
- 右侧旧 Tab 面板（Terminal/Files/Containers）已移除，terminal 和 tasks 成为一等 inspector。
- 工作区从桌面覆盖抽屉改为 inspector/sheet，容器控制和 workspace 不再同时挤占右侧区域。
- Header 新增 `toggle-runtime-tasks`、`toggle-terminal` 事件；CommandPalette 保留 `open-runtime-tasks` 入口。
