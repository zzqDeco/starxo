...
## 2. 核心职责
- 全局命令面板，提供键盘优先的会话、设置、工作区、终端、运行任务、SSH 连接和模式切换入口。

## 3. 输入与输出
- 输入来源: `Props.show`、session/chat/connection stores
- 输出结果: `update:show`、`open-settings`、`open-workspace`、`open-runtime-tasks`、`open-terminal`
...
- 旧历史里的 `Default Session` 在命令面板中显示为本地化的未命名会话，避免暴露变量式默认名。

## 5. 变更影响面
- 顶部 Header 的命令入口和 `Cmd/Ctrl+K` 均打开该组件。
- 新增工作区、终端和运行任务打开命令，需要父组件处理 `open-workspace` / `open-terminal` / `open-runtime-tasks`。
