...
## 2. 核心职责
- 统一 SSH 错误文本提取。
- 针对 macOS app 访问局域网 SSH 主机时报 permission-like dial failure 的场景，提供 Local Network 权限风险判断。
...
## 4. 关键实现细节
- 仅在 macOS 平台、错误包含 `no route to host` / `network is unreachable` / `operation not permitted` / `permission denied`、host 属于 localhost/private/link-local/mDNS 等本地网络地址时返回 true。
- 不修改后端错误语义，不吞掉原始错误，避免掩盖真实网络或认证失败。
- 不提供 `tccutil` reset 指引或系统设置自动变更，权限恢复仍由用户在系统设置中完成。
